...
int CompruebaColumnTimeout (fsm_t* this) {
	...
	result = (flags_player & FLAG_TMR_TIMEOUT);
...
int key_pressed (fsm_t* this) {
	...
	piLock (PLAYER_FLAGS_KEY);
	result = (flags_player & FLAG_KEY_PRESSED);
	piUnlock (PLAYER_FLAGS_KEY);
...
void process_key (fsm_t* this) {
	...
	switch(p_teclado->teclaPulsada.col){
		...
		case COL_3:
			...
			p_teclado->teclaPulsada.row = ROW_1;

			piLock (PLAYER_FLAGS_KEY);
			flags_player |= FLAG_NOTA_TIMEOUT;
			piUnlock (PLAYER_FLAGS_KEY);
			printf("COMIENZA NOTA /n");
